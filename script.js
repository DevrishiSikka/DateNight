...
function renderMovies() {
    ...
    movies.forEach((movie) => {
      ...
      movieCard.className = "movie-card"; // Make sure the card has the correct class
  
      // Tooltip content with movie overview
      const tooltipContent = document.createElement("div");
      tooltipContent.className = "tooltip-content";
      tooltipContent.innerHTML = `<p>${movie.overview}</p>`; // Set the movie overview here
  
      movieCard.appendChild(tooltipContent); // Add the tooltip to the movie card
      ...
      const movieCardHTML = `
        ...
        <div class="p-4 h-5/5 flex flex-col text-center">
          ...
          <!-- Select Button -->
          <button 
              class="flex-grow-[3] bg-gray-800 text-white py-2 rounded-md hover:bg-gray-700 transition-colors select-btn"
      ...
      movieCard.innerHTML += movieCardHTML; // Append the HTML content
      movieListContainer.appendChild(movieCard); // Add the movie card to the container
...
searchInput.addEventListener("input", () =>
  fetchMovies(searchInput.value, genreFilter.value, 1)
);
